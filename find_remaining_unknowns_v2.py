...
def infer_screen(product):
    title = product.get("title", "").lower().replace("  ", " ") # Normalize spaces
    ...
    # Inference Rules (Snapshot of MobilesPage.jsx)
    ...
    if 'redmi note 14' in title or 'redmi note 13' in title or 'redmi note 12' in title: return '6.67"'
    ...
    if 'vivo v' in title: return '6.78"'
    if 'redmi a' in title: return '6.52"'
    if 'redmi 15' in title or 'redmi 14' in title or 'redmi note 14' in title: return '6.7"'
    if 'honor 200' in title or 'honor 400' in title or 'honor 90' in title: return '6.7"'
    if 'oscal' in title: return '6.5"'
    if 'blackview' in title: return '6.5"'

    # Regex Fallback - Updated to support comma
    title_upper = title.upper()
    # Updated Regex to verify: comma support
    match = re.search(r'(\d+[.,]\d+|\d+)\s*(?:PO|INCH|\'\'|")|ECRAN\s*(\d+[.,]\d+)', title_upper)
    if match:
        val = match.group(1) or match.group(2)
        return f"Regex Match: {val}" 
