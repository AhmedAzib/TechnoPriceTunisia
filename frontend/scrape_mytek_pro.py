...
def scrape():
    print("VERSION: Pro 1.0 - Appending Mode")
    print("Launching MyTek Professional Laptop Scraper...")
    ...
    base_url = "https://www.mytek.tn/informatique/ordinateurs-portables/pc-portable-pro.html"
    ...
    try:
        for page in range(1, 100):
            ...
            for p in products:
                ...
                pid = f"mk-pro-{slug[:30]}" # Professional prefix
                
                # Check against existing IDs (and potential duplicates from main category)
                # Note: Main category IDs are 'mk-lap-...'. Pro IDs are 'mk-pro-...'.
                # We need to check if the TITLE or LINK exists? 
                # Or just trust that if it's the same product it might be listed twice with different prefixes?
                # User said "make sure they aren already in web". 
                # Let's check for duplicate LINKS to be safe.
                ...
                if is_duplicate or pid in existing_ids:
                    continue
    ...
    print(f"Finished. Added {new_items_count} new professional laptops.")
