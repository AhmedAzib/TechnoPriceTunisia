...
const ComparePage = () => {
  ...
  // HYDRATE DATA: Ensure we are using the LATEST specs (Categories, OS, etc.)
  // by looking up the ID in MASTER_DATA and re-running normalization.
  // This fixes issues where stale data in localStorage doesn't show new fields.
  const hydratedList = useMemo(() => {
    if (!compareList.length) return [];
    
    // Find latest version of each product
    const freshRaw = compareList.map(staleItem => {
      const fresh = MASTER_DATA.find(m => m.id === staleItem.id || m.link === staleItem.link);
      return fresh || staleItem;
    });

    // Re-apply latest normalization rules (Category, GPU, etc.)
  ...
  const specsKeys = [
    ...
    { label: 'Category', path: ['specs', 'category'], compare: false },
    { label: 'System (OS)', path: ['specs', 'os'], compare: false },
  ];
        ...
        <table className="compare-table">
          ...
          <tbody>
            {specsKeys.map((spec, i) => {
              ...
              const rowValues = hydratedList.map(p => getValue(p, spec));
